...
CREATE PROCEDURE [weekly_ops].[SP_OPS_04_UPRO_previousweek]
as 
...
*/
 ...
 select case when [media] = 'Catalog' then 416
   ...
   where [reportweek_YYYYWW]<=(select (ReportCycle_YYYYWW-1) from [JAVDB].[IREPORT_2014].[dbo].[wb_00_reporting_cycle] group by ReportCycle_YYYYWW)
   group by [media]
   
   
if object_id('tempdb..#UPRO_previous') is not null drop table #UPRO_previous
...
if object_id('weekly_ops.OPS_04_UPRO_previousweek') is not null drop table weekly_ops.OPS_04_UPRO_previousweek
select 
...
into weekly_ops.OPS_04_UPRO_previousweek
from #UPRO_previous as a 
...
select * from weekly_ops.OPS_04_UPRO_previousweek
