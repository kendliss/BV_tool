create procedure [QC_stimreport.SP02_CallACCLSales2] as 
...
select [Scorecard_TypeC]
,[TFN]
, SUM(isnull([ytd_ts_local_accl],0)) as [Overall Call ACCL Sales Declines?]
from [javdb].[IREPORT_2014].[dbo].[WB_07_Stim_Report_WOW]
where [TFN] in (select [TFN] from #pidWOWcall_ACCL)
group by [Scorecard_TypeC]
,[TFN]
having SUM(isnull([ytd_ts_local_accl],0))<0
