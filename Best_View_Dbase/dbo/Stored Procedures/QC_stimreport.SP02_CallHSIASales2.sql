create procedure [QC_stimreport.SP02_CallHSIASales2] as 
--Call HSIA sales
...
select [Scorecard_TypeC]
,[TFN]
, SUM(isnull([ytd_ts_uvrs_HSIA],0)) as [Overall Call HSIA Sales Declines?]
from [javdb].[IREPORT_2014].[dbo].[WB_07_Stim_Report_WOW]
where [TFN] in (select [TFN] from #pidWOWcall_HSIA)
group by [Scorecard_TypeC]
,[TFN]
having SUM(isnull([ytd_ts_uvrs_HSIA],0))<0