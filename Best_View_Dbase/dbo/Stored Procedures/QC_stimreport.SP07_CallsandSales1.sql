create procedure [QC_stimreport.SP07_CallsandSales1] as 
--Check for campaigns with calls and 0 sales
select campaign_type as [Scorecard_Type]
...
having SUM(isnull(ytd_ts_local_accl,0)+ isnull(ytd_ts_ld,0)+ isnull(ytd_ts_dsl_reg,0)+ isnull(ytd_ts_dsl_ip,0)
	+ isnull(ytd_ts_uvrs_tv,0)+ isnull(ytd_ts_cing,0)+ isnull(ytd_ts_uvrs_voip,0)+ isnull(ytd_ts_uvrs_hsia,0)
	+ isnull(ytd_ts_dsl_dry,0)+ isnull(ytd_ts_dish,0)+ isnull(ytd_TS_DLIFE,0))=0
