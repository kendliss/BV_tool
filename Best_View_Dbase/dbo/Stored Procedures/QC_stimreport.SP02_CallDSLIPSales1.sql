...
CREATE procedure [dbo].[QC_stimreport.SP02_CallDSLIPSales1] as
--Call DSL ip sales
...
,a.[total_quantity]
,a.[ytd_ts_dsl_ip]
,case when [ParentID] IN (select [ParentID] from #pidWOWcall_DSLip group by [ParentID]) then 'Y'
	else 'N'
...
from [javdb].[IREPORT_2014].[dbo].[WB_07_Stim_Report_WOW] as a 
	left join (select [TFN], count(parentid)as Share# from [javdb].[IREPORT_2014].[dbo].[WB_07_Stim_Report_WOW] where TFN is not null group by [TFN]) as b
	on a.tfn=b.tfn
where a.[TFN] in (select [TFN] from #pidWOWcall_DSLip)
group by [Scorecard_TypeC]
,a.[TFN]
,a.[Campaign_Name]
,a.[TFN_Description]
