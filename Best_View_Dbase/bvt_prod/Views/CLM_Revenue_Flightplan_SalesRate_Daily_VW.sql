...
DROP VIEW bvt_prod.CLM_Revenue_Flightplan_SalesRate_Daily_VW
GO

CREATE VIEW [bvt_prod].[CLM_Revenue_Flightplan_SalesRate_Daily_VW]
as
...
(select 
	...
	, idkpi_type_FK
	, idProduct_LU_TBL_FK
	, case when idkpi_type_FK=3 then 0
		when tfn_ind=-1 and idkpi_type_FK=1 then Sales_Rate
		...
		else isnull(Sales_Rate,0)
		end as Sales_Rate
	, InHome_Date
	, idTarget_Rate_Reasons_LU_TBL_FK
from bvt_prod.CLM_Revenue_Flight_Plan_VW as A
	
	left join (SELECT * FROM [bvt_prod].[Sales_Rate_Start_End_FUN](9)) as B on A.idProgram_Touch_Definitions_TBL_FK=B.idProgram_Touch_Definitions_TBL_FK
	and InHome_Date between Sales_Rate_Start_Date and b.END_DATE) as SalesRate_Join
---End Join KPI and Flight Plan	

	left join (SELECT * FROM [bvt_prod].[Response_Daily_Start_End_FUN](9)) as B 
...
---End Join Daily Percentages
	...
	left join (SELECT * FROM [bvt_prod].[Sales_Curve_Start_End_FUN](9)) as C
	...
	left join (SELECT * FROM [bvt_prod].[Dropdate_Start_End_FUN](9)) as D
