drop view [bvt_prod].[Movers_Drag_Forecast_VW]
GO

CREATE VIEW [bvt_prod].[Movers_Drag_Forecast_VW]
	AS 
	...
	SELECT 
		...
		case when [idDrag_Method_LU_TBL_FK]=1 then [Metric]*[Day_Percent] 
		  ...
		  FROM (SELECT [Date], MediaMonth, MediaMonth_year as media_year, iso_week as Media_week, [Day_Percent], FV_Calls 
				...
				left join (select [Day_of_Week],[Day_Percent],[Daily_Start_Date],[END_DATE]  from [bvt_prod].[Response_Daily_Start_End_FUN]('Movers')
