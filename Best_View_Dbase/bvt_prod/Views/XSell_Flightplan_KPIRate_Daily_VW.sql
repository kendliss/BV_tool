drop VIEW  [bvt_prod].[XSell_Flightplan_KPIRate_Daily_VW]
go

CREATE view [bvt_prod].[XSell_Flightplan_KPIRate_Daily_VW]
as
...
from [bvt_prod].[XSell_Flight_Plan_VW] as A
	...
	left join (SELECT * FROM [bvt_prod].[KPI_Rate_Start_End_FUN]('X-Sell')) as B on A.idProgram_Touch_Definitions_TBL_FK=B.idProgram_Touch_Definitions_TBL_FK
...
---End Join Daily Percentages
	...
	left join (SELECT * FROM [bvt_prod].[Response_Curve_Start_End_FUN]('X-Sell')) as C
