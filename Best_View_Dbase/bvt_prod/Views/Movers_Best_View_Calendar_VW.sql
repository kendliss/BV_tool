drop view [bvt_prod].[Movers_Best_View_Calendar_VW]
GO

CREATE VIEW [bvt_prod].[Movers_Best_View_Calendar_VW]
	AS 
	select 
...
----END OF COMPLEX Best View case statement-------------------------------------------------------------
	...
	(SELECT 
	  ...
	  from bvt_prod.Movers_Best_View_Forecast_VW a
      ...
      , a.[Product_Code]) as forecast
		...
		full join 
			...
			where CV_Submission in('Movers 2016 Submission Adj 20160422','Movers CV Restatement Aug 2015','Movers Alt Media 2017 Submission 20170124') --extract_date='2015-08-26'
