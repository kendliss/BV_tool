--DROP VIEW [bvt_prod].[CLM_Revenue_Best_View_VW]
--GO

CREATE VIEW [bvt_prod].[CLM_Revenue_Best_View_Calendar_VW]
AS 
	select 
...
----END OF COMPLEX Best View case statement-------------------------------------------------------------
	...
	(SELECT 
	  ...
	  from bvt_prod.CLM_Revenue_Forecast_VW a
      ...
      , a.[Media_YYYYWW]) as forecast
		...
		full join 
			...
			where CV_Submission in ('UCLM Rev 2016 Submission 20160331 v2', 'CLM Revenue Jan-Jun 2017 Submission 20161219')
			GROUP BY [id_Flight_Plan_Records_FK], [idProgram_Touch_Definitions_TBL_FK], [Campaign_Name], [InHome_Date], 
			[Media_Year], [Media_Month], [Media_Week], [Calendar_Year], [Calendar_Month],  [Calendar_Year], [Calendar_Month],  [KPI_TYPE], [Product_Code],
			[Touch_Name], [Program_Name], [Tactic], [Media], [Audience], [Creative_Name], [Goal], [Offer], [Campaign_Type], [Channel],
