drop view [bvt_prod].[Email_Flightplan_Volume_Forecast_VW]
GO


CREATE view [bvt_prod].[Email_Flightplan_Volume_Forecast_VW]
as 
select idFlight_Plan_Records
	...
	from 
		(select idFlight_Plan_Records, idProgram_Touch_Definitions_TBL_FK, idVolume_Type_LU_TBL_FK, idTarget_Rate_Reasons_LU_TBL_FK, 
			...
			from [bvt_prod].[Email_Flight_Plan_VW] left join dim.Media_Calendar_Daily on InHome_Date=Media_Calendar_Daily.Date) as flighting
		...
		left join (SELECT * FROM [bvt_prod].[Target_adjustment_start_end_FUN](11)) as Target_adjustment_start_end
		...
		left join (SELECT * FROM [bvt_prod].[CPP_Start_End_FUN](11)) AS CPP_Start_End on flighting.idProgram_Touch_Definitions_TBL_FK=CPP_Start_End.idProgram_Touch_Definitions_TBL_FK
