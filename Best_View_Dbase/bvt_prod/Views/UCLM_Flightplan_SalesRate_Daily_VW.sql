...
from bvt_prod.UCLM_Flight_Plan_VW as A
	...
	left join (SELECT * FROM [bvt_prod].[Sales_Rate_Start_End_FUN](3)) as B on A.idProgram_Touch_Definitions_TBL_FK=B.idProgram_Touch_Definitions_TBL_FK
	and InHome_Date between Sales_Rate_Start_Date and b.END_DATE) as SalesRate_Join
---End Join KPI and Flight Plan	

	left join (SELECT * FROM [bvt_prod].[Response_Daily_Start_End_FUN](3)) as B 
...
---End Join Daily Percentages
	...
	left join (SELECT * FROM [bvt_prod].[Sales_Curve_Start_End_FUN](3)) as C
	...
	left join (SELECT * FROM [bvt_prod].[Dropdate_Start_End_FUN](3)) as D
