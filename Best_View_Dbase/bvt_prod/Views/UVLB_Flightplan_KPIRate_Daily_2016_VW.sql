...
from [bvt_prod].[UVLB_Flight_Plan_VW] as A
	...
	left join (SELECT * FROM [bvt_prod].[KPI_Rate_Start_End_FUN](1)) as B on A.idProgram_Touch_Definitions_TBL_FK=B.idProgram_Touch_Definitions_TBL_FK
	AND InHome_Date between Rate_Start_Date and b.END_DATE) as KPI_Join
---End Join KPI and Flight Plan	

	left join (SELECT * FROM [bvt_prod].[Response_Daily_Start_End_FUN](1)) as B 
...
---End Join Daily Percentages
	...
	left join (SELECT * FROM [bvt_prod].[Response_Curve_Start_End_FUN](1)) as C
...
----------End  Weekly Response Curve and Media Calendar		
	...
	left join (SELECT * FROM [bvt_prod].[Target_adjustment_start_end_FUN](1)) as Target_adjustment_start_end
