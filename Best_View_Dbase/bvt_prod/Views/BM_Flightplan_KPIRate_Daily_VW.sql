CREATE VIEW [bvt_prod].[BM_Flightplan_KPIRate_Daily_VW]
as
...
from [bvt_prod].[BM_Flight_Plan_VW] as A
	...
	left join (SELECT * FROM [bvt_prod].[KPI_Rate_Start_End_FUN]('BM')) as B on A.idProgram_Touch_Definitions_TBL_FK=B.idProgram_Touch_Definitions_TBL_FK
...
---End Join Daily Percentages
	...
	left join (SELECT * FROM [bvt_prod].[Response_Curve_Start_End_FUN]('BM')) as C
