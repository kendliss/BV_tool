CREATE VIEW [bvt_prod].[BM_Flightplan_SalesRate_Daily_VW]
as
...
from [bvt_prod].[BM_Flight_Plan_VW] as A
	...
	left join (SELECT * FROM [bvt_prod].[Sales_Rate_Start_End_FUN]('BM')) as B on A.idProgram_Touch_Definitions_TBL_FK=B.idProgram_Touch_Definitions_TBL_FK
	and InHome_Date between Sales_Rate_Start_Date and b.END_DATE) as SalesRate_Join
---End Join KPI and Flight Plan	

	left join (SELECT * FROM [bvt_prod].[Response_Daily_Start_End_FUN]('BM')) as B 
...
---End Join Daily Percentages
	...
	left join (SELECT * FROM [bvt_prod].[Sales_Curve_Start_End_FUN]('BM')) as C
