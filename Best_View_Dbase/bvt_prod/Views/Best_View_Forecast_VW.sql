...
Create view bvt_prod.Best_View_Forecast_VW
as
...
from bvt_prod.Flight_Plan_Records as FPR
...
from bvt_prod.FlightplanSalesForecast
 left join bvt_prod.Product_LU_TBL
		on FlightplanSalesForecast.idProduct_LU_TBL_FK=Product_LU_TBL.idProduct_LU_TBL)
...
from bvt_prod.FlightplanKPIForecast
 left join bvt_prod.KPI_Types
		on FlightplanKPIForecast.idkpi_types_FK=KPI_Types.idKPI_Types)
...
(select idFlight_Plan_Records
	...
	, Volume as Forecast
from bvt_prod.Flightplan_Volume_Forecast_VW)) as metricsa) as metrics
...
(select idProgram_Touch_Definitions_TBL, Touch_Name, Program_Name, Tactic, Media, Audience, Creative_Name, Goal, Offer_Description, Campaign_Type
