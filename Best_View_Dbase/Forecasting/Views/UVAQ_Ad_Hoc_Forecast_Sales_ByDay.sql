...
create view [Forecasting].[UVAQ_Ad_Hoc_Forecast_Sales_ByDay]
 as select Ad_Hoc_flightplan_ID,
	   Forecast_Week_Date,
   	   Forecast_DayDate,
   	   Product_FK,
  ...
  (select A.Ad_Hoc_flightplan_ID,
   ...
   from Forecasting.[UVAQ_AdHoc_Flightplan_Forecast_ByDay] as A
  
  INNER JOIN
  
  ---Subquery to pull in the necessary information about the touch type----------------------------------  
  (SELECT Ad_Hoc_flightplan_ID, Media_Type, 
    ...
    Where A.Touch_Type_FK=B.Touch_Type_ID and B.Media_Type_FK=C.Media_Type_ID and B.Audience_FK=D.Audience_ID   
     ...
     ON A.Ad_Hoc_flightplan_ID=flight_plan_record.Ad_Hoc_flightplan_ID) as response_daily
